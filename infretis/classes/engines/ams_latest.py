...
class AMSEngine(EngineBase):#, metaclass=Singleton):
    ...
    def set_mdrun(self, md_items):
        """Remove or rename?"""
        # if self.exe_dir == '.':
        print('AMS setting exe_dir:', md_items)
    ...
    def _extract_frame(self, traj_file, idx, out_file):
        ...
        if out_file in self.states or out_file in self.oldstates:
        ...
        if traj_file in self.states:
            ...
            print("banan2",self.states.keys(),self.oldstates)
    ...
    def modify_velocities(
        ...
        kin_old = system.ekin
        print('state:', self.states.keys())
        print('oldstate:', self.oldstates)
        for state in list(self.states.keys()):
            if state in self.oldstates:
                print('delete:', state)
                self._deletestate(state)
        self.oldstates = list(self.states.keys())
        print('new oldstate:', self.oldstates)
        ...
        if vel_settings.get('aimless', False):
            ...
            if state_name in self.states:
                if state_name != genvel:
                    ...
                    self._deletestate(genvel) 
                    self._copystate(state_name, genvel, idx=idx)
                else:
                    # Delete gen vel if existent
                    print('genvel deleted', genvel)   
            else:
                print("genvel exists", os.path.exists(genvel))
                self._copystate(state_name, genvel, idx=idx)
