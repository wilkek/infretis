...
class AMSEngine(EngineBase):#, metaclass=Singleton):
    ...
    def set_mdrun(self, md_items):
        ...
        # self.exe_dir = engine_dic[self.worker.workerdir]
        # self.md_items = md_items
    ...
    def _extract_frame(self, source, dest, idx=None):
        ...
        """
        # if os.path.exists(dest): # file must never be there before PrepareMD
        #     self._removefile(dest)
  

        if source in self.states:
            logger.debug('AMS extracting frame: %s, %i -> %s', source, idx, dest)
            self._copystate(source, dest, idx=idx)
            self.worker.MolecularDynamics(dest, nsteps=0, setsteptozero=True) # Writes traj file
        else:
            logger.debug('AMS extracting frame from disk: %s, %i -> %s', source, idx, dest)
            rkf = RKFTrajectoryFile(source)
            rkf.store_mddata()
            ...
            self.worker.PrepareMD(dest)
            self.worker.CreateMDState(dest, molecule)
            if 'Velocities' in rkf.mddata: 
                vel = rkf.mddata['Velocities']
                vel = np.reshape(vel, (-1, 3)) # RKFTrajectoryFile returns 1D array
                self.worker.SetVelocities(dest, vel, dist_unit='bohr', time_unit='fs') # Units used in rkf file

            state = self.worker.MolecularDynamics(dest, nsteps=0, setsteptozero=True) # Also writes frame into dest
            self._add_state(dest, state[0])
