...
class AMSEngine(EngineBase): #, metaclass=Singleton):
    ...
    def __init__(self, input_path, timestep, subcycles):
        ...
        if len(random_velocities_method) > 0:
           logger.info('AMS setting velocity generation method to "%s"', random_velocities_method)
    ...
    def set_mdrun(self, md_items):
        ...
        logger.info(f"self.exe_dir {self.exe_dir} md_items['exe_dir'] {md_items['exe_dir']}")
        print('----------------------------------------------------------------------------------------------')
        print('----------------------------------------------------------------------------------------------')
        delete_states = []
        ...
        for state in delete_states:
            ...
            try:
                ...
                # print('state not deleted: ' + state)
                pass
        gc.collect()

        # List all objects and their basic details
        print("### All Objects in Memory ###")
        self.list_objects_with_references()

        # Find objects of specific type
        print("\n### Objects of Type 'list' ###")
        lists = self.find_objects_of_type(dict)
        with open(f'{self.exe_dir}list_objects.txt', 'a') as f:
            f.write(f"Steppel {self.count}\n")
            self.count += 1
            for lst in lists:
                f.write(f"{lst}\n")
        ...
        #     self._deletestate(state)
        print('-----------------------------------------------')
        print('remaining states:', self.states.keys())
        print('----------------------------------------------------------------------------------------------')
        print('----------------------------------------------------------------------------------------------')
        

    def list_objects_with_references(self):
        """
        List all objects currently tracked by the garbage collector along with their type
        and reference counts.
        """
        objects = gc.get_objects()
        print(f"Total objects tracked by gc: {len(objects)}\n")
        
        for obj in objects[:50]:  # Limit output to 50 objects for readability
            try:
                obj_type = type(obj).__name__
                ref_count = sys.getrefcount(obj)  # Reference count of the object
                obj_repr = repr(obj)[:100]  # Truncate representation for readability
                print(f"Type: {obj_type}, Ref Count: {ref_count}, Object: {obj_repr}")
            except Exception as e:
                print(f"Could not process object: {e}")

    def find_objects_of_type(self, target_type):
        """
        Find all objects of a specific type currently tracked by gc.
        """
        objects = gc.get_objects()
        matching_objects = [obj for obj in objects if isinstance(obj, target_type)]
    ...
    def _extract_frame(self, traj_file, idx, out_file):
        ...
        if traj_file in self.states:
            logger.info('AMS extracting frame: %s, %i -> %s', traj_file, idx, out_file)
            ...
            logger.info('AMS extracting frame from disk: %s, %i -> %s', traj_file, idx, out_file)
    ...
    def _propagate_from(
        ...
        logger.info(f'AMS internal traj renaming: {traj_file} -> {traj_file+"_0"}')
        ...
        for i in range(path.maxlen):
            ...
            msg_file.flush()
            if stop:
                logger.info('AMS propagation ended at %i. Reason: %s', i, status)
                if i == 0:
    ...
    def modify_velocities(
        ...
        if vel_settings.get('aimless', False):
            ...
            logger.info('Generating velocities for %s, idx=%s', state_name, idx)
            prefix = self.ens_name + str(os.getpid()) + "_" + str(counter())
            genvel = os.path.join(self.exe_dir, f"gen_vel_{prefix}." + self.ext)
            # print('statename', state_name)
            # print('self.statename', self.states.keys())
            ...
            # print("genvel", self.worker.workerdir, genvel)
    ...
    def _copystate(self, source, dest, idx=None):
        ...
        if idx is None:
            ...
            logger.info('AMS Copying traj snap to snap: %s, %i -> %s', source, idx, dest)
